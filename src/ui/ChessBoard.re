...
let reducer = (action, state) => {
  switch (action) {
  | Active(square) => {...state, active: square}
  | TryMoveFrom(square) =>
...
module Square = {
  ...
  let createElement =
    ...
    let onMouseOver = _ => {
      Revery.Log.info("Square:", "active Some");
      dispatch(Active(Some(square)));
    };
    ...
    let onMouseLeave = _ => {
      Revery.Log.info("Square:", "active None");
    ...
    let onMouseDown = _ => {
      ...
      | NoPiece => ()
      | _ =>
        Revery.Log.info("Square:", "mouse down");
        let pieceEl = <Piece dragging=true piece />;
        Drag.startDragging(
          pieceEl,
          () => {
            Revery.Log.info("Square:", "try move from");
    ...
    <View onMouseOver onMouseLeave onMouseDown onMouseEnter>
